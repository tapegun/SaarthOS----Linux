...
/* tuxctl_ldisc_put()
 * Write bytes out to the device. Returns the number of bytes that 
 * were actually written. This may be less than the number requested
 * if the line discipline's internal buffer is full. It returns -1
 * on error.
 */
